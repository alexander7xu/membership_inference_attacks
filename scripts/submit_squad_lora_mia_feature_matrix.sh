...
case "$mode" in
  ...
  olmo-full)
    ...
    ;;
  *)
    printf 'unsupported feature-matrix mode: %s\n' "$mode" >&2
    exit 2
    ;;
esac
...
job_name="squad_lora_mia_matrix_${model}_${stamp}"
...
job_command+="; exec bash ./scripts/run_squad_lora_mia_feature_matrix.sh $model"
