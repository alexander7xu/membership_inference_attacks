...
CONFIG_NAME=squad_lora_rmia_iid_prompt_unique_control
OUTPUT_ROOT=outputs/squad_lora_rmia_iid_prompt_unique_control

run_stage() {
  local stage=$1
  if [[ "$stage" == "attack" || "$stage" == "attack_control" ]]; then
...
test -f "$model_root/control/mixed_distribution_online_rmia/comparison.json"
test -f "$model_root/analysis/relative_log_likelihood/relative_log_likelihood_ecdf.png"
printf '[%s] model=%s IID shadow-distribution online RMIA complete\n' \
