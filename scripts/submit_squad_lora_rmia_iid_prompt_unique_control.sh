...
case "$mode" in
  ...
  olmo-full)
    ...
    ;;
  *)
    printf 'unsupported IID RMIA mode: %s\n' "$mode" >&2
    exit 2
    ;;
esac
...
job_name="squad_lora_rmia_iid_prompt_unique_${model}_${stamp}"
...
job_command+="; exec bash ./scripts/run_squad_lora_rmia_iid_prompt_unique_control_formal.sh $model"
